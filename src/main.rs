...
fn main() {
    ...
    let conn = pkt_udp::PktConn::connect("127.0.0.1:9004").unwrap();
    conn.set_frame_timeout(Some(Duration::from_millis(500)))
        .unwrap();

    app.add_plugins(DefaultPlugins)
        .insert_resource(Connection(conn))
...
#[derive(Resource)]
struct Connection(pkt_udp::PktConn);
...
fn example_control_system(
    ...
    if input.pressed(KeyCode::M) {
        ...
        conn.0.send(&bytes::Bytes::from(jv.to_string())).unwrap();
        conn.0.recv().unwrap();
        // send
        let jv = json::object! {
            "action": "send",
    ...
    conn.0.send(&bytes::Bytes::from(jv.to_string())).unwrap();
    let pkt = conn.0.recv().unwrap();
    let msg = String::from_utf8(pkt).unwrap();
